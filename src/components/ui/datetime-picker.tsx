...
const timePickerInputBase =
  "p-1 inline tabular-nums h-fit border-none outline-none select-none content-box caret-transparent rounded-sm min-w-8 text-center focus:bg-foreground/10 focus-visible:ring-0 focus-visible:outline-none";
...
>(
  (
    ...
    return (
      <div
        className={cn(
          "flex w-full items-center border p-1",
          className,
          "gap-1 rounded-xl border-input selection:bg-transparent selection:text-foreground",
        )}
                ...
                {group?.length
                  ? group.map((unit, j) => (
                      <React.Fragment key={unit}>
                        <Input
                          className={cn(timePickerInputBase, "!h-12 flex-1", {
                            "min-w-10": unit === "years",
                            "bg-secondary": unit === "am/pm",
                          })}
